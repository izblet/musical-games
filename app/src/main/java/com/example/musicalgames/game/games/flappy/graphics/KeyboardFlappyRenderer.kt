...
class KeyboardFlappyRenderer(
    ...
    override fun draw(
        ...
        pipes.forEach { (_, rectTop, rectBottom) ->
            rectTop?.let { drawKeyboardTiles(canvas, it, gameRect, screenWidth, screenHeight, whitePaint, blackPaint) }
            rectBottom?.let { drawKeyboardTiles(canvas, it, gameRect, screenWidth, screenHeight, whitePaint, blackPaint) }
        }
